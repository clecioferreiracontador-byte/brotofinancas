...
function calcularAlertasDoUsuario(data, hojeStr){
  ...
  (data.lancamentos || []).filter(l => l.tipo === 'divida' && Number(l.parcelas) > 0 && l.vencimento).forEach(l => {
    ...
    if(dias <= NOTIF_DIAS_ANTECEDENCIA){
      alertas.push({ texto: `Parcela de dívida (${l.descricao || 'Dívida'}) vence em ${dias} dia(s)`, urgente: dias < 0 });
    }
  });
  ...
  (data.provisoes || []).forEach(p => {
    ...
    if(dias <= NOTIF_DIAS_ANTECEDENCIA){
      ...
      alertas.push({ texto: `${rotulo} (${alvo || 'sem categoria'}) vence em ${dias} dia(s)`, urgente: dias < 0 });
    }
  });
  ...
  if(orc){
    ...
    Object.keys(orc.categorias || {}).forEach(cat => {
      ...
      if(limite > 0 && gasto > limite){
        alertas.push({ texto: `Orçamento de ${cat} ultrapassado`, urgente: true });
      }
    });
    const gastoTotal = Object.values(gastoPorCat).reduce((s, v) => s + v, 0);
    if(orc.total > 0 && gastoTotal > orc.total){
      alertas.push({ texto: 'Orçamento total do mês ultrapassado', urgente: true });
    }
  }
...
async function main(){
  ...
  for(const usuarioDoc of usuariosSnap.docs){
    ...
    const corpo = alertas.length === 1
      ? alertas[0].texto
      : `${alertas.length} alertas pendentes${urgentes ? ` (${urgentes} vencido(s))` : ''} — abra o Broto para ver.`;
    ...
    console.log(`Broto: ${alertas.length} alerta(s) enviados para usuário ${uid}, ${resposta.successCount}/${tokens.length} entregues.`);
  }
}
